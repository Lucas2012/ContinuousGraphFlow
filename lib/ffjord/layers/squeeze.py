...
class SqueezeLayer(nn.Module):
    ...
    def _downsample(self, x, logpx=None):
        head_shape = x.shape[:-3]
        tail_shape = x.shape[-3:]

        squeeze_x = squeeze(x.view((np.prod(head_shape),) + tail_shape), self.downscale_factor)
        tail_shape = squeeze_x.shape[-3:]
        if logpx is None:
            return squeeze_x.view(head_shape + tail_shape)
        else:
            return squeeze_x.view(head_shape + tail_shape), logpx

    def _upsample(self, y, logpy=None):
        head_shape = y.shape[:-3]
        tail_shape = y.shape[-3:]

        unsqueeze_y = unsqueeze(y.view((np.prod(head_shape),) + tail_shape), self.downscale_factor)
        tail_shape = unsqueeze_y.shape[-3:]
        if logpy is None:
            return unsqueeze_y.view(head_shape + tail_shape)
        else:
            return unsqueeze_y.view(head_shape + tail_shape), logpy
