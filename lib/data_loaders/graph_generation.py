...
class Graph_sampler_flow(torch.utils.data.Dataset):
    def __init__(self, G_list, max_num_node=None, embedding_size=None):
        self.adj_all = []
        self.len_all = []
    ...
    def __getitem__(self, idx):
        ...
        graph        = np.zeros((self.n, self.embedding_size))
        graph_matrix = np.zeros((self.n, self.n))  # here zeros are padded for small graph
        graph_mask   = np.zeros((self.n))

        length       = adj_copy.shape[0]

        # get graphs, graph_matrix, graph_masks
        graph_normal = Normal(torch.zeros(self.n * self.embedding_size), 0.3)
        graph                          = graph_normal.sample().view(self.n, self.embedding_size)
        graph[length:, :]              = 0
        graph_matrix[:length, :length] = adj_copy
        graph_mask[:length]            = 1

        return graph, graph_matrix, graph_mask



########## use pytorch dataloader
class DualGraph_sampler_flow(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        adj_copy          = self.adj_all[idx].copy()
        dual_graph        = np.zeros((int(self.n * (self.n - 1) / 2), 1))
        dual_graph_matrix = np.zeros((int(self.n * (self.n - 1) / 2), int(self.n * (self.n - 1) / 2)))  # here zeros are padded for small graph
        dual_graph_mask   = np.zeros(int(self.n * (self.n - 1) / 2))

        K               = adj_copy.shape[0]
        graph_size      = K

        # get graphs, graph_matrix, graph_masks
        G               = nx.from_numpy_matrix(adj_copy)
        dual_G          = nx.line_graph(G)
        dual_nodes      = dual_G.nodes()
        dual_nodes_list_ = [[ele[0], ele[1]] for ele in dual_nodes] 
        dual_nodes_list  = torch.tensor( dual_nodes_list_ + [[-1,-1]] * (self.n*(self.n-1)//2 - len(dual_nodes_list_)) )

        # get dense graph
        dense_G        = nx.from_numpy_matrix(1 - np.eye(K))
        dual_dense_G   = nx.line_graph(dense_G)
        dual_dense_adj = nx.adjacency_matrix(dual_dense_G).todense()
        dual_dense_nodes      = dual_dense_G.nodes()
        dual_dense_nodes_list_ = [[ele[0], ele[1]] for ele in dual_dense_nodes] 
        dual_dense_nodes_list  = torch.tensor( dual_dense_nodes_list_ + [[-1,-1]] * (self.n*(self.n-1)//2 - len(dual_dense_nodes_list_)) )

        edge_existence = [[int(ele in dual_nodes_list_)] for ele in dual_dense_nodes_list_]

        dual_graph[:K*(K-1)//2, :]                  = np.array(edge_existence)
        dual_graph_matrix[:K*(K-1)//2, :K*(K-1)//2] = dual_dense_adj
        dual_graph_mask[:K*(K-1)//2]                = 1

        return dual_graph, dual_graph_matrix, dual_graph_mask, dual_dense_nodes_list, graph_size



########## use pytorch dataloader
class DualGraph_sampler_flow_sparse(torch.utils.data.Dataset):
    def __init__(self, G_list, max_num_node=None):
        assert(1==0)
        self.adj_all = []
        self.len_all = []
    ...
    def __getitem__(self, idx):
        ...
        dual_G          = nx.line_graph(G)
        dual_nodes      = dual_G.nodes()
        dual_nodes_list = [[ele[0], ele[1]] for ele in dual_nodes] 
        dual_nodes_list = torch.tensor( dual_nodes_list + [[-1,-1]] * (self.n*(self.n-1)//2 - len(dual_nodes_list)) )

        # get dense graph
        dense_G        = nx.from_numpy_matrix(1 - np.eye(K))
        dual_dense_G   = nx.line_graph(dense_G)
        #dual_dense_adj = nx.adjacency_matrix(dual_dense_G).todense()
        dual_sparse_adj = nx.to_scipy_sparse_matrix(dual_dense_G)
        dual_sparse_adj = coo_matrix(dual_sparse_adj)

        dual_graph[:K*(K-1)//2, :]                  = 1
        dual_graph_mask[:K*(K-1)//2]                = 1

        dual_sparse_value  = dual_sparse_adj.data
        dual_sparse_indice = np.vstack((dual_sparse_adj.row, dual_sparse_adj.col))

        return dual_graph, dual_sparse_value, dual_sparse_indice, dual_graph_mask, dual_nodes_list, graph_size


########## use pytorch dataloader
class BipartiteGraph_sampler_flow(torch.utils.data.Dataset):
    def __init__(self, G_list, max_num_node=None):
        assert(1==0)
        self.adj_all = []
        self.len_all = []
    ...
    def __getitem__(self, idx):
        adj_copy        = self.adj_all[idx].copy()
        edge_graph      = np.zeros((int(self.n * (self.n - 1) / 2), 1))
        node_graph      = np.zeros((int(self.n * (self.n - 1) / 2), 1))
        E2V_matrix      = np.ones((int(self.n), int(self.n * (self.n - 1) / 2)))  # here zeros are padded for small graph
        V2E_matrix      = np.ones((int(self.n * (self.n - 1) / 2), int(self.n)))  # here zeros are padded for small graph

        e_mask          = np.zeros(int(self.n * (self.n - 1) / 2))
        v_mask          = np.zeros(int(self.n))

        K_v              = adj_copy.shape[0]
        ...
        return dual_graph, E2V_matrix, V2E_matrix, e_mask, v_mask, dual_nodes_list, K_v, K_e


def dual_sparse_collate_fn(batch):
    all_dual_graph         = [] 
    all_dual_sparse_value  = []
    all_dual_sparse_indice = [] 
    all_dual_graph_mask    = [] 
    all_dual_nodes_list    = []
    all_graph_size         = []

    for i, (dual_graph, dual_sparse_value, dual_sparse_indice, dual_graph_mask, dual_nodes_list, graph_size) in enumerate(batch):
      all_dual_graph.append(torch.from_numpy(dual_graph).unsqueeze(0))
      all_dual_sparse_value.append(torch.from_numpy(dual_sparse_value))
      all_dual_sparse_indice.append(torch.from_numpy(dual_sparse_indice))
      all_dual_graph_mask.append(torch.from_numpy(dual_graph_mask).unsqueeze(0))
      all_dual_nodes_list.append(dual_nodes_list.unsqueeze(0))
      all_graph_size.append(torch.FloatTensor([graph_size]).unsqueeze(0))

 
    all_dual_graph         = torch.cat(all_dual_graph)
    #all_dual_sparse_value  = torch.cat(all_dual_sparse_value)
    #all_dual_sparse_indice = torch.cat(all_dual_sparse_indice)
    all_dual_graph_mask    = torch.cat(all_dual_graph_mask)
    all_dual_nodes_list    = torch.cat(all_dual_nodes_list)
    all_graph_size         = torch.cat(all_graph_size)
