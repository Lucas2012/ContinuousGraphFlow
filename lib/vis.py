...
def draw_layout(vocab, objs, boxes, masks=None, size=256,
                show_boxes=True, bgcolor='white', output_filename='sample.png'):
...
def draw_box(box, color, text=None):
  ...
  if text is not None:
    ...
    #ty = 0.5 * (y0 + y1)
    #ty = y0
    plt.text(tx, ty, text, va='center', ha='center', fontsize=20)
...
if __name__ == '__main__':
  ...
  a = torch.load('test_draw.pth')

  vocab = a['vocab']
  objs  = a['objs']
  boxes = a['boxes']

  for i in range(0, objs.size(0)):
    import IPython; IPython.embed()
    try:
      draw_layout(vocab, objs.long()[i], boxes[i])
    except:
      continue
